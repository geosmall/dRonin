...
#define PIOS_MPU_SPI_HIGH_SPEED  11000000
#define PIOS_MPU_SPI_LOW_SPEED    1000000
...
struct pios_mpu_dev {
	...
	pios_spi_t spi_driver_id;                  // Handle to the spi communication driver */
	pios_i2c_t i2c_driver_id;                  // Handle to the i2c communication driver */
...
static int32_t PIOS_MPU_Mag_WriteReg(uint8_t reg, uint8_t data)
{
	...
	if (PIOS_MPU_WriteReg(PIOS_MPU_SLV4_REG_REG, reg) != 0) { printf("**** SLAVE ERR ****\n");
		return -1;}
...
{
	printf("pios_bone_mpu: PIOS_MPU_I2C_Init called....\n");
...
{
	printf("pios_bone_mpu: PIOS_MPU_SPI_Init called....\n");
...
static void PIOS_MPU_Task(void *parameters)
{
	...
	while (true) {
		...
		if (mpu_dev->use_mag == true)
			usleep(1375);  // internal mag sleep time
		else
